...
//	    String str = "[3,9,20,null,null,15,7]";
	    public static TreeNode3 mkTree(String str) {
	        int []arr = StrToIntArray(str);
	        TreeNode3 []nodes = new TreeNode3[arr.length + 1];
	        for (int i = 1; i < nodes.length; i++) {
	            if (arr[i - 1] != Integer.MAX_VALUE) {
	                nodes[i] = new TreeNode3(arr[i - 1]);
	            }else {
	                nodes[i] = null;
	            }
	        }
...
//    }
	    ...
	    public ArrayList<Integer> preorderTraversal(TreeNode3 root) {
	        ArrayList<Integer> returnList = new ArrayList<Integer>();
	 
	        if(root == null)
	            return returnList;
	 
	        Stack<TreeNode3> stack = new Stack<TreeNode3>();
	        stack.push(root);
	 
	        while(!stack.empty()){
	            TreeNode3 n = stack.pop();
